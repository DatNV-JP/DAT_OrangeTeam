<template>
  <div>
    <el-table :data="listOrder" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="130" align="center" label="Ngày tạo">
        <template slot-scope="scope">
...
<script>
import { cancelOrder, getListOrderUser } from '@/api/order'
...
export default {
  ...
  methods: {
    ...
    getListOrder() {
      ...
      getListOrderUser(params).then(res => {
    ...
    detailOrder(data) {
      this.$router.push('/user/tracking-page/' + data.id)
    }
  }
}
</script>
