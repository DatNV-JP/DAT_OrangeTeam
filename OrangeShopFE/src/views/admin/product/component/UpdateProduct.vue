    ...
    <ValidationObserver v-slot="{ invalid, handleSubmit }">
      <el-form ref="productForm" :model="productForm" label-position="left" label-width="15%">
        <div class="editor-container pb-3">
          <dropzone id="myVueDropzone" :remove-file-all="removeAllFile" :url="uploadUrl+'/upload-files'" :default-img="defaultImgs" :remove-file-name="removeFileName" @dropzone-removedFile="dropzoneR" @dropzone-successmultiple="dropzoneS" />
              ...
              <div v-for="(input, index) in inputs" :key="index">
                <ValidationProvider v-slot="{ errors }" :rules="(index!==inputs.length-1 || index === 0)?'required|uniqueValues:'+inputs:'|uniqueValues:'+inputs">
                  <el-form-item :error="messageError('Phân loại hàng ', errors[0])">
                    ...
                    <el-input v-model="inputs[index]" placeholder="Blue/ XL" readonly @input="addInput(index)">
              ...
              <div v-for="(input, index) in inputs1" :key="index">
                <ValidationProvider v-slot="{ errors }" :rules="(index!==inputs1.length-1 || index === 0)?'required|uniqueValues:'+inputs1:'|uniqueValues:'+inputs1">
                  <el-form-item :error="messageError('Phân loại hàng ', errors[0])">
                    ...
                    <el-input v-model="inputs1[index]" placeholder="Blue/ XL" readonly @input="addInput1(index)">
                      <template v-slot:append>
        ...
        <template v-if="classifyVisible">
          <el-card class="mb-3">
            <el-row :gutter="20">
              ...
              <el-col :span="6">
                <el-form-item label="Giá mặc định:" label-width="42%">
                  <el-input-number
            ...
            >
              ...
              >
                <template v-slot="scope">
                  ...
                  >
                    ...
                    <i v-if="!scope.row.img && !scope.row.images" class="el-icon-plus avatar-uploader-icon" />
                    <img v-if="scope.row.images && !scope.row.img" :src="uploadUrl + '/productDetailImages/' + scope.row.images" class="avatar" alt="">
                  </el-upload>
              ...
              >
                <template v-slot="{row}">
                  <ValidationProvider v-slot="{ errors }" rules="required|double">
                    <el-input
...
export default {
  ...
  created() {
    getProductDetail(this.$route.params).then(res => {
      this.productForm = JSON.parse(JSON.stringify(res.data))
    }).finally(async() => {
      await this.getAttributes()
      this.tableDataArr = this.productForm.productDetails
      this.listDefaultImg = this.productForm.defaultImage.split(',')
      // console.log(this.listDefaultImg)
      for (let i = 0; i < this.listDefaultImg.length; i++) {
        this.defaultImgs.push(`${this.uploadUrl}/productImage/${this.listDefaultImg[i]}`)
      }
    })
  ...
  methods: {
    async getAttributes() {
      ...
      const attribute1 = this.listVariation[0].name
      const attribute2 = this.listVariation[1].name
      const colors = new Set()
      const sizes = new Set()
      for (const detail of this.productForm.productDetails) {
        for (const variationOption of detail.variationOptions) {
          if (variationOption.variation.name === attribute1) {
            colors.add(variationOption.value)
          }
          if (variationOption.variation.name === attribute2) {
            sizes.add(variationOption.value)
          }
        }
      }
      this.inputs = Array.from(colors)
      this.inputs1 = Array.from(sizes)
      // this.inputs.push('')
      // this.inputs1.push('')
    },
    confirmExit(event) {
    ...
    addProduct() {
      this.$swal({
        title: 'Bạn có chắc chắn?',
        text: 'Bạn có chắc chắn sửa sản phẩm này!',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if (result.isConfirmed) {
          ...
          updateProduct(this.productForm).then(res => {
            ...
            this.$router.push('/product-admin/product-management')
            this.hideLoading()
          })
        }
      })
    ...
    async getVariation() {
      await getListVariation({ categoryId: this.productForm.categoryId }).then(res => {
